...
// @require      https://cdn.jsdelivr.net/npm/hls.js@1.0.7/dist/hls.min.js
// @include      *
...
// @license      GPL License
// ==/UserScript==
//fly改动867行 furls 与 flinks 新增直接观看
(function() {
	
    'use strict';
	var newVersion = 'v5.3.6';
		...
		GM_config.init(  
		{
			...
			fields:
			{
				...
				http_ua: 
				{
					...
					type: 'textarea',
					default: 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36 115Browser/23.9.2'
				},
	...
	function batchList(key,down){
			...
			if(down){
				download(el,i).then(function(data){
					if(data[0]){
						...
						furls.push({num:data[2],href:data[0],name:data[1],sha:data[3]});
					}else{
		...
		var time5 = setInterval(function(){
			...
			if(end){
				...
				console.log(hrefs)
				window.prompt("请复制",flinks) //改动 加入同步变量flinks furls
				clearInterval(time5); 
			};
		}, 100);
		setTimeout(function(){
	...
	function list_menu(){
		setTimeout(function(){
			...
			if(G.get('file_Down') && $sed.length==1){
				...
				file['txt']=$sed.attr('title').replace(/\s/g,'&nbsp;');
			...
			if($('li[file_type="0"].selected').length==0){
				if($sed.length>0){ 
					...
					if(G.get('ASX_list')){
						if($('#js_operate_box').is(':hidden')){
							$down.siblings('.115down').addBack().eq('-1').after('<li class="115ASXlist" title="生成M3U播放列表" style="display:list-item;"><a href="javascript:;"><i class="icon-operate"></i><span>播放列表</span></a></li>');
						}else if($('.115ASXlist').length<1){
							$('#js_operate_box li:eq(0)').after('<li class="115ASXlist" title="生成M3U播放列表"><i class="icon-operate"></i><span>播放列表</span></li>');
						};
					};
				};
				if($sed.length>1){
	...
	$(document).ready(function(){
		if(localHref.indexOf('https://115.com/') != -1) {	
			...
			var herfv = 'li[file_type="1"][file_mode="9"]:not([paly_button="1"])';
	...
	$(document).ready(function(){
		...
		if (G.get('offline_Down') && localHref.indexOf('115.com') == -1){
			if(localHref.match(/[0-9]mag\.net/) != null){//无极磁链ØMagnet
				right_menu();
